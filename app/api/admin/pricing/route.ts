...
export async function GET(_request: NextRequest) {
  ...
  try {
    const ref = adminDb.collection(PRICING_SETTINGS_COLLECTION).doc(PRICING_SETTINGS_DOC_ID);
    const snapshot = await ref.get();
    const raw = snapshot.data() || {};

    const pricing = normalizeSubscriptionPricing(raw);
    const updatedAt =
...
export async function PUT(request: NextRequest) {
  ...
  try {
    ...
    return Response.json({
      success: true,
      pricing: normalizeSubscriptionPricing(raw),
      meta: {
        updatedAt:
          raw.updatedAt && typeof raw.updatedAt.toDate === "function"
            ? raw.updatedAt.toDate().toISOString()
